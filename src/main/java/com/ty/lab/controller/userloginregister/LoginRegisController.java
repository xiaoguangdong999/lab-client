...
public class LoginRegisController {
    ...
    public Map register(@RequestBody TbAccountinfo user) {
        ...
        tbAccountinfo.setUsername(user.getUsername());
        List<TbAccountinfo> allByExample = accountinfoService.findAllByExample(tbAccountinfo);
        if (allByExample.size()>0) {
            map.put("code", "fail");
            map.put("message", "该工号已注册,请登录");
            return map;
        }
